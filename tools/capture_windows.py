...
def capture_window(window, path: Path) -> None:
    ...
    try:
        ...
        if (
            gdi32.GetDIBits(memory_dc, bitmap, 0, height, data, ctypes.byref(header), 0)
            != height
        ):
            raise RuntimeError("GetDIBits did not read the complete window")
        picture = Image.frombuffer(
            "RGB", (width, height), data.raw, "raw", "BGRX", 0, 1
        )
...
def prepare_capture_desktop() -> None:
    ...
    if width < 1600 or height < 1050:
        raise RuntimeError(
            "Capture needs a desktop of at least 1600x1050; offscreen Tk regions would be blank"
        )
